"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AppModule = void 0;
const common_1 = require("@nestjs/common");
const mongoose_1 = require("@nestjs/mongoose");
const auth_module_1 = require("./auth/auth.module");
const user_module_1 = require("./user/user.module");
const database_providers_1 = require("./database.providers");
const database_module_1 = require("./database/database.module");
const movie_module_1 = require("./movie/movie.module");
const movie_service_1 = require("./movie/movie.service");
const movie_providers_1 = require("./movie/movie.providers");
const movie_controller_1 = require("./movie/movie.controller");
const user_controller_1 = require("./user/user.controller");
const jwt_1 = require("@nestjs/jwt");
const movie_model_1 = require("./movie/movie.model");
const subscription_module_1 = require("./subscription/subscription.module");
require('dotenv').config();
let AppModule = class AppModule {
};
exports.AppModule = AppModule;
exports.AppModule = AppModule = __decorate([
    (0, common_1.Module)({
        imports: [
            jwt_1.JwtModule.register({
                secret: 'JWT_SECRET',
                signOptions: { expiresIn: '1d' },
            }),
            mongoose_1.MongooseModule.forRoot('mongodb://localhost/movieCrudApp', {}),
            mongoose_1.MongooseModule.forFeature([{ name: 'Movie', schema: movie_model_1.MovieModel }]),
            auth_module_1.AuthModule,
            user_module_1.UserModule,
            database_module_1.DatabaseModule,
            user_module_1.UserModule,
            movie_module_1.MovieModule,
            subscription_module_1.SubscriptionModule,
        ],
        controllers: [movie_controller_1.MovieController, user_controller_1.UserController],
        providers: [...database_providers_1.databaseProviders, movie_service_1.MovieService, ...movie_providers_1.movieProviders],
    })
], AppModule);
//# sourceMappingURL=app.module.js.map